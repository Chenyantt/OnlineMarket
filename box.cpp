		...
		if (talkers[i] == id) {
			for (int j = 0; j < messages_num; ++j) {
				if (messages[j].sender == id) {
					cout << FRONT_DARKGREEN << "          "<<messages[j].time << RESET << endl;
					cout << id << endl;
	...
	for (int j = 0; j < messages_num; ++j) {
		if (messages[j].content.find(input) != -1) {
			...
			if (messages[j].sender == owner->getid()) {
				cout <<'('<< messages[j].time << ')' <<endl;
				cout <<REVERSE << FRONT_GREEN<< "发给" << messages[j].receiver << ":" <<RESET <<REVERSE<< messages[j].content<<RESET << endl;
			}
			else {
				cout <<'('<< messages[j].time << ')' <<endl;
				cout << REVERSE << FRONT_RED << "来自" << messages[j].sender << ":" << RESET << REVERSE << messages[j].content << RESET << endl;
			}
		}
	}
...
void Box::send() {
	...
	while (id == "") getline(cin, id);
	if (id == owner->getid()) {
		cout << FRONT_RED << "不能给自己发送消息" << RESET << endl;
		return;
	}
	cout << "请输入要发送的内容：";
	string input;

...
void CheckMessage() {
	...
	for (map<string, Box>::iterator iter = boxes.begin(); iter != boxes.end(); ++iter) {
		for (int j = 0; j < iter->second.messages_num; ++j) {
			...
			cout << '('<< iter->second.messages[j].time<<')'<< endl;
			cout << REVERSE << FRONT_GREEN <<iter->second.messages[j].sender << "发送给" <<RESET<<REVERSE << iter->second.messages[j].receiver<< ':' <<iter->second.messages[j].content <<RESET << endl;
			}
		}
	}
...
void SearchMessage() {
	...
	for (int i = 0; i < iter->second.messages_num; ++i) {
		if (iter->second.messages[i].sender == input) {
			cout <<'('<< iter->second.messages[i].time <<')'  <<endl;
			cout << REVERSE << FRONT_GREEN << "发送给" << iter->second.messages[i].receiver << "："
				<<RESET <<REVERSE<< iter->second.messages[i].content <<RESET<< endl;
		}
		else {
			cout <<'('<< iter->second.messages[i].time <<')'  <<endl;
			cout << REVERSE << FRONT_RED << "来自于" << iter->second.messages[i].sender << "："
				<< RESET << REVERSE << iter->second.messages[i].content << RESET << endl;
		}
	}
	cout << "******************************************" << endl;
